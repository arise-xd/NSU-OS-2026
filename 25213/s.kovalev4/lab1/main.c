...
int main(int argc, char *argv[])
{
    ...
    for (int i = options_count - 1; i >= 0; i--)
    {
        ...
        case 'p':
            if (print_process_info() != 0) // fork | exec
                return 1;
            break;
        ...
        case 'U':
            if (set_ulimit(options[i].argument) != 0) // как можно менять ulimit
                return 1;
            break;
        ...
        case 'C':
            if (set_core_limit(options[i].argument) != 0) //как можно менять core limit
                return 1;
            break;
